...
- A new experimental driver `apc_microlink` is now available for APC
  Smart-UPS models that use the proprietary MicroLink USB HID protocol
  (USB VID/PID 0x051D/0x0003), such as the SMC1000C.
...
- The `upsdrvquery*` semi-internal API for talking on the driver socket was
  updated with default handling for SIGPIPE. If your consumer wants to catch
  it themselves, instead of using `errno=EPIPE` after a failed call, you can
